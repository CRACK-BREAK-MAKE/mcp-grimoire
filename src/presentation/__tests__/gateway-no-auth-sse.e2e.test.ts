/**
 * Gateway E2E Test: No Auth SSE Server
 *
 * PURPOSE:
 * Validates intent resolution without authentication over SSE
...
describe('Gateway E2E - No Auth SSE', () => {
  let serverProcess: ChildProcess;
  let gateway: GrimoireServer;

  const serverPort = FASTMCP_PORTS.GATEWAY_NO_AUTH_SSE_TIER1; // 8024
  const serverUrl = `http://localhost:${serverPort}/sse`;
  const testSpellName = 'gateway-no-auth-sse';

  let grimoireDir: string;
  let spellFilePath: string;

  beforeAll(async () => {
    grimoireDir = setupTestGrimoireDir('gateway-no-auth-sse');
  ...
  it('should auto-spawn No Auth SSE server via resolve_intent', async () => {
    const toolsBeforeSpawn = gateway.getAvailableTools();
    expect(toolsBeforeSpawn.length).toBe(2);

    const query = 'use sse to get cpu usage memory stats and disk usage';
